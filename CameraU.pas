...
uses
  ActiveX, Classes, Windows, Graphics, Messages, Forms, Jpeg,
  Global, MatrixU, ImageU, PvAPI,
...
const
  ...
  FakeFrameMsg = WM_USER+2;
  StartMsg     = WM_USER+3;
  TerminateMSg = WM_USER+4;
...
type
  ...
  TCameraInfo = packed record
    ...
    Enabled      : Boolean;
    Reserved     : array[1..255-16-16-1] of Byte;
  end;
...
//    UndistMap       : PIplImage;}
  ...
  public
    ...
    SmoothOddBmp  : TBitmap;
    SmoothEvenBmp : TBitmap;
...
uses
  Dialogs, SysUtils, BmpUtils, Math, Controls, FileCtrl, Routines,
  {OpenCV,} CalFile, FileU, CamSettingsFrmU,
  GLDraw;
...
procedure TCamera.SetToDefault;
begin
  Enabled:=True;
  Exposure.Mode:='Manual';
  Exposure.Value:=20000;
  ...
  WhiteBalance.Mode:=AutoMode;
  WhiteBalance.Red:=100;
  WhiteBalance.Blue:=100;
  WhiteBalance.Rate:=50;

  Smooth:=False;
end;

procedure TCamera.ReadFromStream(Stream:TFileStream);
begin
  Stream.Read(Enabled,SizeOf(Enabled));
  Stream.Read(Exposure,SizeOf(Exposure));
  Stream.Read(Gain,SizeOf(Gain));
  Stream.Read(Pose,SizeOf(Pose));
  Stream.Read(KInfo,SizeOf(KInfo));
  Stream.Read(FlipImage,SizeOf(FlipImage));
  Stream.Read(MirrorImage,SizeOf(MirrorImage));
  Stream.Read(WhiteBalance,SizeOf(WhiteBalance));
  Stream.Read(Smooth,SizeOf(Smooth));
end;

procedure TCamera.WriteToStream(Stream:TFileStream);
begin
  Stream.Write(Enabled,SizeOf(Enabled));
  Stream.Write(Exposure,SizeOf(Exposure));
  Stream.Write(Gain,SizeOf(Gain));
  Stream.Write(Pose,SizeOf(Pose));
  Stream.Write(KInfo,SizeOf(KInfo));
  Stream.Write(FlipImage,SizeOf(FlipImage));
  Stream.Write(MirrorImage,SizeOf(MirrorImage));
  Stream.Write(WhiteBalance,SizeOf(WhiteBalance));
  Stream.Write(Smooth,SizeOf(Smooth));
end;

procedure TCamera.SmoothBmp;
begin
  if Odd(FrameCount) then begin
    DrawSmoothBmp(OddBmp,SmoothOddBmp);
  end
  else DrawSmoothBmp(EvenBmp,SmoothEvenBmp);
end;

procedure TCamera.DrawSubtractedBmp;
begin
  if Smooth then begin
    SubtractBmpAsmAbs32Bit(SmoothOddBmp,SmoothEvenBmp,SubtractedBmp);
  end
  else SubtractBmpAsmAbs32Bit(OddBmp,EvenBmp,SubtractedBmp);
end;
...
begin
  ...
  else if Msg.Msg=FakeFrameMsg then begin
    MeasureFPS;
    DrawStatic(Bmp);
    if Assigned(FOnNewFrame) then FOnNewFrame(Self);
    Msg.Result:=1;
...
// init vars
  ...
  OddBmp:=CreateImageBmp;
  EvenBmp:=CreateImageBmp;
  SmoothOddBmp:=CreateImageBmp;
  SmoothEvenBmp:=CreateImageBmp;
...
// images
  if Assigned(OddBmp) then OddBmp.Free;
  if Assigned(EvenBmp) then EvenBmp.Free;
  if Assigned(SmoothOddBmp) then SmoothOddBmp.Free;
  if Assigned(SmoothEvenBmp) then SmoothEvenBmp.Free;
...
// count the cameras
  Count:=PvCameraList(@CameraInfo[1],1,ConnectedCount);

// open the first one if we found any
  if (Count>0) and (ConnectedCount>0) then begin
...
//  LinePadding:=0;
  ...
  try
    ...
    Bmp.Width:=iFrame.Width;
    Bmp.Height:=iFrame.Height;
    ...
    if FlipImage then for Y:=iFrame.Height-1 downto 0 do begin
      ...
      if MirrorImage then begin
        for Y:=0 to iFrame.Height-1 do begin
          Line:=Bmp.ScanLine[Y];
          for X:=iFrame.Width-1 downto 0 do begin
            I:=X*3;
            Line^[I]:=BufferPtr^;
...
function TCamera.Bmp: TBitmap;
begin
  if Odd(FrameCount) then Result:=OddBmp
  else Result:=EvenBmp;
end;
...
begin
  ...
  if RotateImage then begin
    ...
    RenderTexturedRectangle3(0,0,W,H,1);
...
//    Sleep(1000000);
    ...
    PostMessage(FHandle,FakeFrameMsg,0,0);
